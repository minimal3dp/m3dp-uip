...
async def main():
    ...
    validator = VisionValidator(
        ...
        dataset_path="backend/validation_data"
    )
    ...
    for i, image_path in enumerate(image_paths, 1):
        ...
        try:
            ...
            logger.info(f"    Got: {result.predicted_defect} (confidence: {result.confidence:.2f})" if result.confidence else f"    Got: {result.predicted_defect}")
